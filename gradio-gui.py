...
def runAnalysis(input_path, species_file, confidence, sensitivity, overlap, species_list_choice, species_list_file, lat, lon, week, sf_thresh, output_type, locale, batch_size, threads):
    locale = locale.lower()
    ...
    cfg.SPECIES_LIST_FILE = species_file.name
...
if __name__ == '__main__':
    ...
    with gr.Blocks(css=r".d-block .wrap {display: block !important;}") as demo:
        ...
        with gr.Row():
            species_list_radio = gr.Radio([_CUSTOM_SPECIES, _PREDICT_SPECIES, "disable"], value=_CUSTOM_SPECIES, label="Species list", elem_classes="d-block")

            with gr.Column(visible=False) as position_row:
                lat_number = gr.Slider(minimum=-90, maximum=90, value=0, step=1, label="Latitude")
                lon_number = gr.Slider(minimum=-90, maximum=90, value=0, step=1, label="Longitude")
                week_number = gr.Slider(minimum=0, maximum=56, value=0, step=1, label='Week')
                sf_thresh_number = gr.Slider(minimum=0.01, maximum=0.99, value=0.03, step=0.01, label="Location filter threshhold")

            species_file_input = gr.File(value='example/species_list.txt', file_types=['.txt'])
        ...
        with gr.Row():
            batch_size_number = gr.Number(precision=1, label="Batch size", value=1)
            threads_number = gr.Number(precision=1, label="Threads", value=4)

        locale_radio = gr.Radio(["EN", "ES", "DE", "FR"], value="EN", label="Locale")

        species_list_radio.change(show_species_choice, inputs=[species_list_radio], outputs=[position_row, species_file_input, empty_col])

        inputs = [audio_input, species_file_input, confidence_slider, sensitivity_slider, overlap_slider, species_list_radio, species_file_input, lat_number, lon_number, week_number, sf_thresh_number, output_type_radio, locale_radio, batch_size_number, threads_number]
    ...
    window = webview.create_window('BirdNET-Analyzer', url, width=1024, height=960)
